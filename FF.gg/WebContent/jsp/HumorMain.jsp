					...
					<table id="ListTable" class="table-hover">
						...
						<tr>
							<td colspan="6" align="center" style="font-size: 20px; padding-top: 20px; padding-bottom: 20px;"><c:if test="${start != 1 }">
									<a href="Humor_Main.do${hstring }page=1">[처음]</a>
									<a href="Humor_Main.do${hstring }page=${start-1 }">[이전]</a>
								</c:if>
								<c:forEach begin="${start }" end="${end }" var="i">
									<c:choose>
										<c:when test="${i==current }"> [${i }]</c:when>
									<c:otherwise>
										<a href="Humor_Main.do${hstring }page=${i }">[${i }]</a>
									</c:otherwise>
								</c:choose>
								</c:forEach> <c:if test="${end < last }">
									<a href="Humor_Main.do${hstring }page=${end+1 }">[다음]</a>
									<a href="Humor_Main.do${hstring }page=${last } ">[마지막]</a>
								</c:if></td>
						</tr>
					</table>
				</div>
					...
					<nav class="navbar navbar-expand-sm bg-light navbar-primary">
						<ul class="navbar-nav">
							<li class="nav-item active">
								<a class="nav-link" href="Humor_insertForm.do">글 쓰 기</a>
							</li>
							<li class="nav-item active">
								<a class="nav-link" href="Community_main.do">메 인 으 로</a>
							</li>
							<form class="form-inline" action="Humor_Main.do" method="post">
								<li class="nav-item active">
